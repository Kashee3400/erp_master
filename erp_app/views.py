...
from util.deeplink_utils import (
    KASHEE_SCHEME,
    KASHEE_MEMBERS_PACKAGE,
    KASHEE_PES_PACKAGE,
    KASHEE_SAHAYAK_PACKAGE,
    get_store_link,
)
...
def detect_platform(user_agent: str) -> str:
    ...
    if "android" in ua_lower:
        return "android"
    elif any(device in ua_lower for device in ["iphone", "ipad", "ipod"]):
        return "ios"
    else:
        return "desktop"
...
def open_deep_link(request):
    """
    Universal deep link handler

    Query Parameters:
        target: The kashee:// deep link (required)
        fallback: Optional fallback URL for desktop users
        app: Optional app selector ('members', 'pes', 'sahayak')

    Examples:
        /open?target=kashee://product/123
        /open?target=kashee://news/456&fallback=https://example.com/news/456
        /open?target=kashee://feedback/88&app=pes
    """
    target = request.GET.get("target", "")
    fallback_url = request.GET.get("fallback", "")
    app_selector = request.GET.get("app", "members").lower()

    # Validate target
    if not target or not target.startswith(KASHEE_SCHEME):
        return JsonResponse(
            {
                "error": "Invalid or missing target parameter",
                "example": "/open?target=kashee://product/123",
            },
            status=400,
        )
    ...
    logger.info(
        f"Deep link request: {target} from {request.META.get('HTTP_USER_AGENT', 'Unknown')}"
    )
    ...
    package_map = {
        "members": KASHEE_MEMBERS_PACKAGE,
        "pes": KASHEE_PES_PACKAGE,
        "sahayak": KASHEE_SAHAYAK_PACKAGE,
    }
    package_name = package_map.get(app_selector, KASHEE_MEMBERS_PACKAGE)

    # Platform-specific handling
    if platform == "android":
        return handle_android_deep_link(request, target, package_name, fallback_url)

    elif platform == "ios":
        return handle_ios_deep_link(request, target, package_name, fallback_url)

    else:  # desktop
        return handle_desktop_fallback(request, target, fallback_url)


def handle_android_deep_link(
    request, target: str, package_name: str, fallback_url: str
):
    """
    Handle deep link for Android devices

    Strategy:
    1. Try to open app with Intent
    2. If app not installed, redirect to Play Store
    3. If that fails, show fallback URL
    """
    intent_url = f"intent://{target.replace('kashee://', '')}#Intent;scheme=kashee;package={package_name};end"

    context = {
        "target": target,
        "intent_url": intent_url,
        "play_store_url": get_store_link(package_name, "android"),
...
def handle_ios_deep_link(request, target: str, package_name: str, fallback_url: str):
    """
    Handle deep link for iOS devices

    Strategy:
    1. Try to open app with kashee:// scheme
    2. If app not installed, redirect to App Store
    3. If that fails, show fallback URL
    """
    context = {
        "target": target,
        "app_store_url": get_store_link(package_name, "ios"),
        "fallback_url": fallback_url,
        "platform": "ios",
    }
...
def handle_desktop_fallback(request, target: str, fallback_url: str):
    """
    Handle desktop users

    Strategy:
    1. If fallback URL provided, redirect there
    2. Otherwise show informational page with QR code
    """
    if fallback_url:
    ...
    context = {
        ...
        "fallback_url": fallback_url,
        "platform": "desktop",
        "qr_data": target,  # Can be used to generate QR code
    }
