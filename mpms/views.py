...
class TblfarmercollectionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        mpms_member = Tblfarmer.objects.filter(
        ...
        aggregated_data = (
            queryset.values("dumpdate__year", "dumpdate__month", "period")
            .annotate(
    ...
    def list(self, request, *args, **kwargs):
        ...
        response = {
            "status": status.HTTP_200_OK,
            "message": "Success",
            "data": {"results": paginated_data, "totals": totals},
        }
        return Response(response, status=status.HTTP_200_OK)
...
class OldDataCollectionView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            fy_start_date <= today <= fy_end_date
            or fy_start_date <= provided_date <= fy_end_date
        ):
            ...
            mpms_farmer_collection_data = mpms_farmer_collection.filter(
                dumpdate=provided_date
            )

            aggregates = mpms_farmer_collection.aggregate(
                total_lr=Sum("weightliter"), total_amount=Sum("totalamount")
            )
            unique_dumpdate_count = (
                mpms_farmer_collection.values("dumpdate").distinct().count()
            )

            old_final_aggregated_data = {
                "total_days": unique_dumpdate_count,
                "total_qty": aggregates.get("total_lr", 0),
                "total_payment": aggregates.get("total_amount", 0),
            }
        else:
            mpms_farmer_collection_data = []
            old_final_aggregated_data = {
                "total_days": 0,
                "total_qty": 0,
                "total_payment": 0,
        ...
        response_data = {
            ...
            "message": "success",
            "data": {
                "old_dashboard_data": TblfarmercollectionSerializer(
                    mpms_farmer_collection_data, many=True
                ).data,
    ...
    def get_date_range(self, mpms_member):
        # Logic to determine start_date and end_date based on mccid
        ...
        if int(mpms_member.mccid.mccid) == 4:
            ...
            end_day = 20
        start_date = date(year=timezone.now().year, month=month, day=day)
        end_date = date(year=timezone.now().year, month=month, day=end_day)
